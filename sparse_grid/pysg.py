...
class gridPoint:
    ...
    def pointPosition(self, index, domain=None):
        ...
        if domain is None:
            ...
            for i in range(int(len(index) / 2)):
                coord.append((domain[i][1] - domain[i][0]) * index[2 * i + 1] / 2.0 ** index[2 * i] + domain[i][0])
        return coord
...
class sparseGrid:
    ...
    def evalFunct(self, x):
        ...
        for i in range(self.dim):
            ...
            for j in range(1, self.level + 1):
                ...
                self.evalPerDim[i][j - 1].append(evalBasis1D(x[i], self.evalPerDim[i][j - 1][0], self.domain[i]))
    ...
    def generatePointsRec(self, dim, level, cur_level=None):
        ...
        if dim == 1 and cur_level == level:
            ...
            return cross(basis_cur, self.generatePointsRec(dim - 1, level - cur_level + 1))
        else:
            # crossproduct of this dim indices and other (dim-1) ones
            # since levels left, generate points for higher levels
            return cross(basis_cur, self.generatePointsRec(dim - 1, level - cur_level + 1)) + self.generatePointsRec(dim, level, cur_level + 1)
    ...
    def nodal2Hier1D(self, node, i, j, dim):
        ...
        while left[1] % 2 == 0 and left[0] > 0:
            left = [left[0] - 1, int(left[1] / 2)]
        while right[1] % 2 == 0 and right[0] > 0:
            right = [right[0] - 1, int(right[1] / 2)]
